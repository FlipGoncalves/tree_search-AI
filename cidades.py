...
class Cidades(SearchDomain):
    ...
    def cost(self, city, action):
        ...
        if city != CITY1:
            return None
        for (C1,C2,D) in self.connections:
            if ( (C1==CITY1) and (C2==CITY2) ) or ( (C2==CITY1) and (C1==CITY2) ):
                return D
        return None
    def heuristic(self, city, goal_city):
        return ((self.coordinates[city][1] - self.coordinates[goal_city][1]) ** 2 + (self.coordinates[city][0] - self.coordinates[goal_city][0]) ** 2) ** (1/2)
